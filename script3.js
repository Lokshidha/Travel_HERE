...
function calculateRouteFromAtoB(platform, start, stop) {
    var router = platform.getPublicTransitService(),
        routeRequestParams = {
            origin: start, // Use the coordinates of the start location
            destination: stop, // Use the coordinates of the stop location
...
// Modify the searchLocation function to get the user input and call calculateRouteFromAtoB
function searchLocation(locationType) {
    var userInput;

    if (locationType === 'start') {
        userInput = document.getElementById('start-input').value;
    } else if (locationType === 'stop') {
        userInput = document.getElementById('stop-input').value;
    }

    geocode(platform, userInput, locationType);
}

// Modify the geocode function to pass the location type to the calculateRouteFromAtoB function
function geocode(platform, userInput, locationType) {
        ...
        function (result) {
            ...
            if (locations.length > 0) {
                ...
                if (locationType === 'start') {
                    ...
                    startLocation = position.lat + ',' + position.lng;
                } else if (locationType === 'stop') {
                    // Set the stop location
                    stopLocation = position.lat + ',' + position.lng;
                }

                // Calculate the route if both start and stop locations are set
                if (startLocation && stopLocation) {
                    calculateRouteFromAtoB(platform, startLocation, stopLocation);
                }
            }
        },
        onError
